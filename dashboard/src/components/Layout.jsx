  ...
  return (
    <div className="min-h-screen flex flex-col font-body text-neo-cream relative z-10">
      ...
      <header
        ...
        className={`sticky top-0 z-50 transition-all duration-300 ease-in-out border-b border-neo-cream/10 ${
      ...
      >
        <div className="w-full px-8 h-20 flex items-center justify-between gap-4">
          ...
          <div
            ...
            onClick={() => navigate('/')}
          >
            <span className="font-heading text-4xl text-neo-cream tracking-widest uppercase mt-2">
              {location.pathname.startsWith('/settings') ? t('navSettings') : 
               location.pathname.startsWith('/alerts') ? t('navAlerts') :
               location.pathname.startsWith('/nodes') ? t('navNodes') : 'CROPCALM'}
            </span>
          </div>
          ...
          <div className="hidden md:block absolute left-1/2 -translate-x-1/2">
            <MorphicNavbar
              items={{
                '/': { name: t('navDashboard') },
                '/nodes': { name: t('navNodes') },
                '/alerts': { name: t('navAlerts') },
                '/settings': { name: t('navSettings') },
              }}
            />
          </div>
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={toggleTheme}
              className="bg-neo-cream text-neo-dark rounded-lg p-1.5 transition-all hover:scale-110"
