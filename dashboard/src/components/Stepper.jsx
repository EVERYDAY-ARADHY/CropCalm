...
export default function Stepper({
  ...
  const handleNext = async () => {
    if (!isLastStep) {
      if (onStepSubmit) {
        try {
          await onStepSubmit(currentStep);
        } catch (error) {
          return; // Stop advancing if validation throws
        }
      }
      setDirection(1);
      updateStep(currentStep + 1);
...
function StepIndicator({ step, currentStep, onClickStep, disableStepIndicators }) {
  ...
  return (
    <motion.div onClick={handleClick} className="step-indicator" style={disableStepIndicators ? { pointerEvents: 'none', opacity: 0.5 } : {}} animate={status} initial={false}>
      <motion.div
        variants={{
          inactive: { scale: 1, backgroundColor: 'rgba(244,231,213,0.1)', color: 'rgba(244,231,213,0.4)' },
          active: { scale: 1, backgroundColor: 'var(--color-neo-green-dark)', color: 'var(--color-neo-cream)' },
          complete: { scale: 1, backgroundColor: 'var(--color-neo-green-dark)', color: 'var(--color-neo-cream)' }
        }}
      ...
      >
        {status === 'complete' ? (
          ...
          <span className="step-number font-heading text-lg">{step}</span>
        )}
      </motion.div>
    </motion.div>
  );
...
function StepConnector({ isComplete }) {
  const lineVariants = {
    ...
    complete: { width: '100%', backgroundColor: 'var(--color-neo-green-dark)' }
  };

  return (
    <div className="step-connector" style={{ backgroundColor: 'rgba(244,231,213,0.1)' }}>
      <motion.div
